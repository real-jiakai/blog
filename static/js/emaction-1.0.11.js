...
class EmojiReaction extends s {
  ...
  // Render into the light DOM so all styling comes from the site's external,
  // CSP-controlled stylesheet rather than runtime-created style elements.
  createRenderRoot() {
    return this;
  }
  ...
  render() {
    ...
    const add_label = this.dataset.addLabel || "Add reaction";
    return x`
    <!-- container -->
    <div class="reaction-container ${this?.theme === 'dark' || (this?.theme === 'system' && system_theme === 'dark') ? 'reaction-theme-dark' : 'reaction-theme-light'}">
      <!-- 灰色笑脸 -->
      <div class="reaction-picker-wrap">
        <button type="button" id="start-smile" aria-label="${add_label}" @click="${this._showAvailable}">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" width="1rem" height="1rem" id="start-smile-svg">
            <path
        ...
        <div class="anim-scale-in reaction-available-popup ${this.showAvailable ? 'is-visible' : ''}" @click=${this._closePopup}>
          <!-- reactions available -->
          ${this.availableReactions.map(item => x`
            <button type="button" aria-label="${item.reaction_name}" @click=${this._react} data-name="${item.reaction_name}" class="reaction-available-emoji ${item.meReacted ? 'reaction-available-emoji-reacted' : ''}">${item.emoji}</button>
          `)}
        </div>
      </div>
      <!-- reactions got -->
      <div class="reactions-got">
        ${this.availableReactions.map(item => x`
          <button type="button" aria-label="${item.reaction_name}: ${item.count}" @click=${this._react} data-name="${item.reaction_name}" class="reaction-got ${item.meReacted ? 'reaction-got-reacted' : 'reaction-got-not-reacted'} ${item?.count && item.count > 0 ? '' : 'reaction-got-hidden'}">
            <span class="reaction-got-emoji">${item.emoji}</span><span class="reaction-got-count">${item.count}</span>
          </button>
        `)}
      </div>
    </div>
