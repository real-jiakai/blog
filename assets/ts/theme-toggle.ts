...
(function () {
    ...
    function syncUtterances(effective: 'light' | 'dark'): void {
        ...
        var frame = document.querySelector<HTMLIFrameElement>('.utterances-frame');
    ...
    function markActive(pref: ThemePref): void {
    ...
    var opts = document.querySelectorAll<HTMLElement>('.theme-option');
    for (var i = 0; i < opts.length; i++) {
        opts[i].addEventListener('click', function () {
            var pref = this.getAttribute('data-theme') as ThemePref;
            if (pref === 'system') {
